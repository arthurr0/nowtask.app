...
class IntegrationEventListener {
    ...
    void onCreated(TaskEvents.TaskCreated event) {
        send(IntegrationService.EVENT_TASK_CREATED, event.taskKey(), event.title(), event.actorId());
    }
    ...
    void onAssigned(TaskEvents.TaskAssigned event) {
        ...
        send(IntegrationService.EVENT_TASK_ASSIGNED, event.taskKey(),
                users.findById(event.assigneeId()).map(UserView::name).orElse(""), event.actorId());
    }
    ...
    void onStatusChanged(TaskEvents.TaskStatusChanged event) {
        send(IntegrationService.EVENT_TASK_STATUS_CHANGED, event.taskKey(),
                event.fromStatusLabel() + " \u2192 " + event.toStatusLabel(), event.actorId());
    }

    private void send(String event, String taskKey, String message, UUID actorId) {
        ...
        executor.execute(
                () -> OrganizationContextHolder.runAs(scope, () -> integrations.dispatch(event, taskKey, message, actorId)));
    }
}
